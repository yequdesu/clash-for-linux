...
pub fn render_simple_table(
    ...
    let header_row = Row::new(headers.to_vec()).style(Style::default().fg(CLASH_THEME.muted));
    ...
    let widths: Vec<Constraint> = (0..cols).map(|_| Constraint::Length(18)).collect();
...
pub fn cell_span(text: &str, color: ratatui::style::Color) -> Span<'_> {
    Span::styled(
        text.to_string(),
        Style::default().fg(color).bg(CLASH_THEME.surface),
    )
}
