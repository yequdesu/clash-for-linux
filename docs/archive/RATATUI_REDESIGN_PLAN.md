...
- 状态栏、确认弹窗、Command Palette、Help、Subscriptions 主视图和订阅 add/edit/quick-edit 弹窗的主要可见文案已迁移到统一 i18n key；Help/Command Palette 风险等级会按语言展示；Subscriptions 底部按钮、订阅表单 Save/Cancel、Settings prompt Continue/Cancel、确认弹窗 Confirm/Cancel 的 hitbox 已按翻译后的终端显示列宽注册，避免中文宽字符在 tmux 分屏/窄 pane 中导致点击错位。
