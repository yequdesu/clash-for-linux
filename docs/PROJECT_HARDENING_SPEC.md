...
- systemd unit 不得写死过低 `LimitNPROC`，不得依赖无业务意义的 `ExecStartPre=/usr/bin/sleep`；失败重启必须配置 start limit，避免启动失败时形成无限重启风暴。
- CLI 调用 `systemctl` 失败时必须携带 stdout/stderr 或明确提示 `journalctl -u clashctl`，不得只返回 `exit status 1`。
...
- 桌面/SSH 用户进程数较多时，systemd unit 仍能启动 Mihomo，不会因为 `LimitNPROC` 或 `ExecStartPre` 失败。
