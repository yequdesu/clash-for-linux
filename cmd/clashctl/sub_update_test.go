...
func TestMergeAutoUpdateCrontabAddsCronLine(t *testing.T) {
	...
	if !strings.Contains(got, "/usr/local/bin/clashctl sub update --cron\n") {
...
func TestMergeAutoUpdateCrontabKeepsExistingEnabledLine(t *testing.T) {
	existing := "0 */12 * * * clashctl sub update --cron\n"
	got, changed := mergeAutoUpdateCrontab(existing, "0 */12 * * * /usr/local/bin/clashctl sub update --cron")
	if changed {
